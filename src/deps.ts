export * as fs from 'https://deno.land/std@0.208.0/fs/mod.ts';
export * as path from 'https://deno.land/std@0.208.0/path/mod.ts';
export {default as MurmurHash3} from 'https://deno.land/x/murmurhash@v1.0.0/mod.ts';
export * as base64 from 'https://deno.land/std@0.208.0/encoding/base64.ts';
export {existsSync} from 'https://deno.land/std@0.208.0/fs/mod.ts';
export {deepMerge} from 'https://deno.land/std@0.208.0/collections/mod.ts';
export {serveFile} from 'https://deno.land/std@0.208.0/http/file_server.ts';
export * as velocirouter from 'https://deno.land/x/velocirouter@v0.5.0/mod.ts';
export * as bumble from 'https://raw.githubusercontent.com/dbushell/bumble/v0.9.7/mod.ts';
